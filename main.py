...
# 2. 결과 정의 (MBTI 16가지 타입별 감성적인 파스텔 이미지 테마)
RESULTS = {
    "ISTJ": ("클래식 베이지", "정돈된 미니멀리즘과 베이지 톤의 안정적인 공간. 질서와 평온함이 느껴지는 테마."),
    "ISFJ": ("소프트 핑크", "따뜻한 톤의 핑크와 화이트가 조화된 아늑한 홈 데코. 배려와 안락함이 느껴지는 테마."),
    "INFJ": ("라벤더 퍼플", "은은한 라벤더와 보라색 안개가 낀 몽환적인 새벽 풍경. 깊은 통찰과 신비로움이 느껴지는 테마."),
    "INTJ": ("스카이 그레이", "절제된 스카이 그레이 톤의 모던하고 구조적인 오피스/서재. 논리와 비전이 느껴지는 테마."),
    "ISTP": ("민트 그린", "자유로운 민트 그린과 청량한 하늘 아래의 캠핑 또는 여행 사진. 독립심과 탐험이 느껴지는 테마."),
    ...
    "ESTP": ("밝은 옐로우", "햇살이 가득한 밝은 옐로우 톤의 역동적인 도시 거리 풍경. 활동성과 에너지가 느껴지는 테마."),
...
def calculate_mbti(scores):
    ...
    # 점수가 0보다 크면 첫 번째 문자, 아니면 두 번째 문자
    mbti = ""
...
# 2. 결과 화면
    ...
    theme_title, theme_desc = RESULTS.get(mbti_type, ("테마 미정", "결과를 찾을 수 없습니다."))
    ...
    st.markdown(f"""
        <div class="result-box">
            ...
            <p class="theme-title">💖 어울리는 감성 이미지 테마: 『{theme_title}』</p>
